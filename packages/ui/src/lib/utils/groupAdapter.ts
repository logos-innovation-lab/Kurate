...
export class GroupAdapter extends EventEmitter2 {
  ...
  async sync() {
    ...
    const newIdentityEvents = await this.globalAnonymousFeed.queryFilter(
      this.globalAnonymousFeed.filters.NewIdentity()
    )
    ...
    this._persistGroupIds(Object.keys(data).reduce((map: any, groupId: string) => {
      map[groupId] = groupId
      return map
    }, {}))
  }
